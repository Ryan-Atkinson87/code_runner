...
## Milestone 5: Trackers + Notifications — 2026-06-24

### What was done

The human-in-the-loop communication layer was built across nine issues in three parallel workstreams — Notion tracker sync, the notification service, and the blocker system — giving the engine the ability to keep external trackers current, notify the human of events and blockers, and accept commands back via Telegram without requiring the human to watch the terminal.

The Notion tracker sync workstream started with a scoped API client (`app/notion/client.py`) that auto-discovers databases by title under a workspace page, handles Notion's 429 rate limiting with jittered exponential backoff, and exposes a typed interface for page reads and writes. On top of this, an idempotent GitHub→Notion mirror sync (`app/sync/mirror.py`) reads all GitHub issue and milestone state as the authoritative source and converges the Notion database to match — creating, updating, or skipping each row independently with per-issue error isolation so a partial failure leaves a cleanly re-runnable state. A Social Media Context page updater (`app/sync/social_context.py`) renders the project's current status, recent milestones, and upcoming work from GitHub state, and was wired into the wave driver's hand-off sequence so the Notion page updates automatically at every wave completion.

The notification service workstream built a channel abstraction (`app/notifications/channel.py`) with a `Channel` protocol, a priority-aware dispatcher (`app/notifications/dispatcher.py`) that routes messages to channels based on message kind (instant vs digest) with per-channel failure isolation, and a factory that assembles channels from configuration toggles and secrets. Two outbound channels were implemented: Telegram via the Bot API with full MarkdownV2 escaping of all 18 special characters, and Resend for email digests. Both implement the `Channel` protocol and auto-register in a `ChannelRegistry`. The capstone of this workstream was a Telegram two-way control channel (`app/notifications/telegram_inbound.py`) that polls for inbound messages, authenticates by chat ID, and routes commands through a `CommandRouter` (`app/notifications/telegram_commands.py`) supporting status, pause, resume, override usage, and resolve-blocker commands — each modifying the engine's live state and replying with confirmation.

The blocker system workstream built a structured blocker model and SQLite store (`app/blockers/`) covering the full park/list/resolve lifecycle with severity levels, typed blocker kinds (missing spec, contract conflict, unmet dependency, stuck agent), and resolution response tracking. An escalation engine (`app/engine/escalation.py`) ties blockers to the notification service: when an issue is parked as a blocker, the engine records it in the store and immediately dispatches a notification to all configured channels. Notification failures are isolated — a failed Telegram send doesn't prevent the blocker from being recorded or the wave from continuing with other issues.

### Why it was done

The Specification's sixth principle — human out of the loop until the wave PR — assumes the engine can surface genuine blockers immediately and keep trackers current without human monitoring. Before this milestone, the engine could run autonomously but had no way to communicate: blockers were logged but not escalated, the human had to watch the terminal to know what was happening, and Notion was not updated. Phase 5 closes the feedback loop. The Notion sync keeps the Social Media Context page current for build-in-public content. The notification service means the human learns about blockers and hand-off PRs within seconds, on their phone. The Telegram two-way control means the human can pause, resume, override usage limits, or resolve a blocker from their phone without opening a terminal — the first step toward truly autonomous operation where the human's role is review and decision-making, not monitoring.
